...
from . import model_week_jp_v2 as base
...
V3_MODEL_MODE = "v3_trend_filtered_upside_probability_week_jp"
...
def main() -> None:
    ...
    def parse_args_v3():
        ...
        if args.table == "STOCK_DATA_WEEK_JP":
            args.table = weekly_split_source("JP")
        if args.end_date == static.end_date:
            args.end_date = training_cutoff_date()
        if args.model_out.endswith("model_week_jp_v2.pt"):
            args.model_out = str(Path(__file__).resolve().parents[1] / "models" / "model_week_jp_v3.pt")
        return args
