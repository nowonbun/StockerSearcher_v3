...
def recent_collection_window(today: date | None = None) -> tuple[str, str]:
    """Return the inclusive 31-day window from 30 days ago through today."""
        ...
        for stock in stocks:
            for is_weekly, frequency in ((False, stock_lib.FREQUENCY_TYPE_DAY), (True, stock_lib.FREQUENCY_TYPE_WEEK)):
                if is_weekly and not weekly:
                    continue
                raw = dataset_jp.fetch_stock_raw(driver, f"{stock.code}.T", stock_lib.PERIOD_TYPE_MONTH, 1, frequency, 1)
                if raw is not None:
...
def calculate_indicators(market: str, weekly: bool = True) -> None:
    ...
    for is_weekly in (False, True) if weekly else (False,):
        ...
        for code in codes:
            with psycopg.connect(**static.db_config) as connection, connection.cursor() as cursor:
                cursor.execute(f"SELECT date, open, high, low, close, volume FROM {tables.ohlcv} WHERE code = %s ORDER BY date", (code,))
                frame = pd.DataFrame(cursor.fetchall(), columns=("date", "open", "high", "low", "close", "volume"))
            payload = [(code, *row) for row in _indicator_rows(frame)]
            if not payload:
                continue
